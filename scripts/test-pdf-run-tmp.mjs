...
function drawClientSection(doc, y, client) {
  ...
  if (client.name)    { doc.text(client.name,    MARGIN, cy, { lineBreak: false }); cy += 14; }
  if (client.company) { doc.text(client.company, MARGIN, cy, { lineBreak: false }); cy += 14; }
  if (client.address) {
    for (const line of client.address.split('\n')) {
      if (line.trim()) { doc.text(line.trim(), MARGIN, cy, { lineBreak: false }); cy += 14; }
    }
  }
  if (client.phone) { doc.text(client.phone, MARGIN, cy, { lineBreak: false }); cy += 14; }
  if (client.email) { doc.text(client.email, MARGIN, cy, { lineBreak: false }); }
}
