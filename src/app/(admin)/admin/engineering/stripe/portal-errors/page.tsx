...
export default function StripePortalErrorsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const res = await fetch('/api/admin/engineering/stripe/ops?kind=portal&success=false&limit=100', {
    ...
    <div className="min-h-[calc(100vh-120px)]">
      <div className="mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
        ...
        <header className="flex flex-col gap-2">
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">Billing Portal Errors</h1>
          <p className="text-sm sm:text-base text-gray-600">Failed portal attempts from your system.</p>
        </header>
        ...
        <div className={card}>
          {loading ? (
            ...
            <div className="text-sm text-gray-600">No portal errors recorded.</div>
          ) : (
          ...
          <div className="mt-4 text-[11px] text-gray-500">
            If portal is failing but this is empty, your portal creation code isn’t logging to stripe_ops_log yet.
          </div>
        </div>
      </div>
    </div>
