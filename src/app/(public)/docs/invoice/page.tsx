...
export default function InvoiceGenerator() {
  ...
  const [lineItems, setLineItems] = useState<LineItem[]>([
    { id: '1', description: 'Professional Painting Labour', quantity: 3, unitPrice: 150, total: 450 },
    { id: '2', description: 'Materials (Paint, Primer, Sealant)', quantity: 1, unitPrice: 200, total: 200 },
  ]);
  const [vatRate, setVatRate] = useState(20);
  const [notes, setNotes] = useState('Work completed to specification. Thank you for your business.');
  const [terms, setTerms] = useState('Payment due within 30 days. Bank details on file.');
  ...
  const handleGeneratePDF = () => {
    const invoice = generateDocument('invoice', business, client, lineItems, vatRate, notes, terms);
    alert(`Invoice ${invoice.details.number} generated. PDF download ready (scheduled for next update).`);
  };
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            <h1 className="text-4xl font-bold text-slate-900">Generate Invoice</h1>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-6 sticky top-8">
                ...
                <div className="text-sm space-y-3 mb-6 pb-6 border-b border-gray-200">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-slate-900">{formatDate(new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0])}</p>
                  </div>
                </div>

                <div className="space-y-4 mb-6 pb-6 border-b border-gray-200">
                  <div>
                    <p className="text-xs font-semibold text-gray-600 uppercase mb-1">Bill From</p>
                    <p className="font-semibold text-slate-900">{business.name}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-gray-600 uppercase mb-1">Bill To</p>
